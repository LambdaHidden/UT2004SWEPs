...
if CLIENT then
	...
	SWEP.WepSelectIcon		= surface.GetTextureID( "vgui/ut2004/targetpainter" )
	SWEP.UTBobScale			= .6
...
function SWEP:CanBomb(pos, radius)
	local minzdist = cvars.Number("ut2k4_minbomberheight") or 2048
	--[[if util.QuickTrace(pos, Vector(0,0,minzdist)).HitWorld then return false end
	
	if radius <= 0 then return true end
	
	local MarkRadius = pos+Vector(0,0,128)
	local tr = {
		start = MarkRadius,
		endpos = MarkRadius,
		mins = Vector(-radius, -radius, -100),
		maxs = Vector(radius, radius, 100),
		mask = MASK_SOLID
	}
	...
	local tr = {
		...
		maxs = Vector(radius, radius, minzdist),
		mask = MASK_SOLID
	}
	return !util.TraceHull(tr).Hit
end

function SWEP:SpawnBomber(MarkLocation, BombDir)
	local minzdist = cvars.Number("ut2k4_minbomberheight") or 3055
	local maxzdist = 6111
	local BombDirection = Angle(0, BombDir[2], BombDir[3])
	local BomberStart, BomberStart2, BombTargetCenter, Extent, Temp
	
	Extent = Vector(512,512,320)
	
	local tr1 = util.QuickTrace(MarkLocation, Vector(0,0,maxzdist + Extent.z))
	BombTargetCenter = tr1.HitPos - Vector(0,0,Extent.z)
	if BombTargetCenter.z - MarkLocation.z < minzdist then return false end
	
	local tr2 = util.QuickTrace(BombTargetCenter, BombDirection:Forward() * 10000) -- In the original game, this traces 100000 units and succeeds if NEITHER HITS. In Source, that's impossible.
	BomberStart = tr2.HitPos - BombDirection:Forward()*Extent[1]
	
	local tr3 = util.QuickTrace(BombTargetCenter, BombDirection:Forward() * -10010) -- So we're just spawning the bomber at the edge of the world.
	BomberStart2 = tr3.HitPos + BombDirection:Forward()*Extent[1]
	
	if BomberStart:DistToSqr(BombTargetCenter) < BomberStart2:DistToSqr(BombTargetCenter) then
		Temp = BomberStart
	...
	if SERVER then
		local bomber = ents.Create("ut2004_bomber")
		bomber:SetOwner(self:GetOwner())
		bomber:SetPos(BomberStart)
		bomber:SetAngles((BomberStart2 - BomberStart):Angle())
		bomber:Spawn()
		bomber:Activate()
...
function SWEP:PaintTarget()
	...
	elseif self:GetPaintState() == 1 then
		if !self.MarkLocation then
			...
			self.MarkLocation = self:CanBomb(eyetr.HitPos, 256) and eyetr.HitPos or nil
			self.MarkTime = ct
		else
	...
	elseif self:GetPaintState() == 2 then
		if self.MarkLocation:DistToSqr(eyetr.HitPos) < 930 then
			if ct - self.MarkTime > PaintDuration and self:SpawnBomber(self.MarkLocation, own:EyeAngles()) then
...
function SWEP:ViewModelDrawn(vm)
	...
	if self.LaserBrightness > 0 or self.LaserBrightness2 > 0 then
		local startpos = own:GetShootPos() + own:GetAimVector()*120 + own:GetRight()*16 + own:GetUp()*-13
		-- Draw the laser beam.
...
function SWEP:DrawWorldModel()
	...
	local own = self:GetOwner()
	if self.LaserBrightness > 0 or self.LaserBrightness2 > 0 then
		local startpos = own:GetShootPos() + own:GetAimVector()*48 + own:GetRight()*6 + own:GetUp()*-2
		-- Draw the laser beam.
		render.SetMaterial( LASER )
		self.LaserColor.a = Lerp( self.LaserBrightness, 0, 255 )
		render.DrawBeam(startpos, own:GetEyeTrace().HitPos, 4, 0, 12.5, self.LaserColor)
...
SWEP.ViewModel			= "models/ut2004/onsfullanimations/targetpainter.mdl"
SWEP.WorldModel			= "models/ut2004/onsfullanimations/targetpainter_3rd.mdl"
